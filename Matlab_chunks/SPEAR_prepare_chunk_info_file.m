...
% inputs paths
dataset = 'Dataset_4/';
inpath.dataset='/Volumes/SPEAR_SSD2/SPEAR/InitialRelease'; %'/Volumes/HD FRL/data/EasyComDataset/Main';
inpath.extra = 'Extra';
inpath.main = 'Main';
...
outpath.filename='segments_D4';  % name for output csv
dataset_id='D4'; % dataset ID (value used in the associated column)
...
total_sources = 7; % maximum number of possible particiapnts could be present

nSession=2; % number of sessions
...
for session=1:nSession
    ...
    nFile=length(files);
    for fi=1:nFile
        fprintf('** Session: %d/%d - File: %d/%d\n',session,nSession,fi,nFile);
        file=files(fi);
